          ...
          if source_type=='MIDI':              
            ...
            for x in range(MIDI_notes):
              ...
              if track_his_name.startswith('csedo'):
                root = str(4) + re.search(r'^csedo_(.+)', track_his_name).group()
                root = '"' + root + '"'
                RPR_ShowConsoleMsg(root)
                note_pitch = 65-note_pitch 
                OSC_schedule = 'evaMIDI "' + track_dir_name + '", ' + str(note_start-play_pos) + comma + str(note_dur-play_pos) + comma + str(note_vel) + comma + note_env + comma + 'mtof:i(' + str(note_pitch) + ')\n'
