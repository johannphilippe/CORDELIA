...
note_nums_extracted = []
for f in wav_files:
	#DK Glock_Natural_B3_L1_RR1.wav
	note_name_split = f.split('_')
	note_name = note_name_split[-3]
	
	if note_name_split[-1].split('.')[0] == 'RR1' and note_name_split[-2] == 'L1':
...
for note_name, note_num in note_nums_extracted:
	for i in range(1, 2):
		for j in range(1, 4):
			local_name = f'DK Glock_Natural_{note_name}_L{i}_RR{j}.wav'
