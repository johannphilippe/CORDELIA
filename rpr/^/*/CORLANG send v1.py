...
def OSC_send(action):
  ...
  s.sendto(action, ("localhost", 10015))
  s.close()
          ...
          if source_type=='MIDI':              
            ...
            for x in range(MIDI_notes):
              ...
              if track_his_name.startswith('csedo'):
                ...
                OSC_schedule = 'evaMIDI "' + track_dir_name + '", ' + str(note_start-play_pos) + comma + str(note_dur-item_pos) + comma + str(note_vel) + comma + note_env + comma + icps + '\n'
                ...
                OSC_schedule = 'evaMIDImode "' + track_dir_name + '", ' + str(note_start-play_pos) + comma + str(note_dur-item_pos) + comma + str(note_vel) + comma + note_env + comma + str(division) + comma + str(2) + comma + str(note_pitch) + '\n'
              else:
                OSC_schedule = 'evaMIDI "' + track_dir_name + '", ' + str(note_start-play_pos) + comma + str(note_dur-item_pos) + comma + str(note_vel) + comma + note_env + comma + 'mtof:i(' + str(note_pitch) + ')\n'
  ...
  if RPR_GetPlayState()==1:
    ...
    if csound_stopped == False:
      ...
      for x in range(len(RPR_instr_array)): 
        instr_name = str(RPR_instr_array[x])
        message_stop += 'turnoff2 ' + instr_name + ', 0, 0\n'
        message_stop += 'turnoff3 ' + instr_name + '\n'
      
      for y in range(len(list_instr)):
        instr_name = str(list_instr[y])
        message_stop += 'turnoff3 nstrnum("' + instr_name.replace('\n','') + '")\n'
